...
namespace PUBS\Admin {
    ...
    class RefType_Rest extends \WP_REST_Controller
    {
        /**
         * The namespace.
         *
        ...
        public function register_routes()
        {
            ...
            register_rest_route($this->namespace, '/' . $this->rest_base, array(
                ...
                array(
                    'methods'             => RefType_Rest::READABLE,
                    'callback'            => array($this, 'get_item'),
                    'permission_callback' => array($this, 'get_item_permissions_check'),
                ),
                array(
                    'methods'         => RefType_Rest::EDITABLE,
                    'callback'        => array($this, 'update_item'),
                    'permission_callback' => array($this, 'update_item_permissions_check'),
                    'args'            => $this->get_endpoint_args_for_item_schema(false),
                ),
                array(
                    'methods'         => RefType_Rest::CREATABLE,
                    'callback'        => array($this, 'create_item'),
                    'permission_callback' => array($this, 'create_item_permissions_check'),
                    'args'            => $this->get_endpoint_args_for_item_schema(true),
                ),
                array(
                    'methods'         => RefType_Rest::DELETABLE,
        ...
        public function get_item($request)
        {
            if ($request['id'] == '') {
                ...
                $reftype = RefType::GetAll($request);
            } else {
                // Call static function Get (use :: to reference static function)
                $reftype = RefType::Get($request['id']);
            }

            if (!is_wp_error($reftype)) {
                $response = rest_ensure_response($reftype);
                return $response;
            } else {
                $error_string = $reftype->get_error_message();
                return new \WP_Error('RefType_Get_Error', 'An error occured: ' . $error_string, array('status' => 400));
            }
        }
